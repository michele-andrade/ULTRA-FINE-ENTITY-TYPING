...
def train(args, train_dataset, model, tokenizer):
    ...
    torch.cuda.amp
    torch.backends.cudnn.max_split_size_mb = 10
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            indicator = torch.tensor(np.ones(len(dat_true), dtype=np.float16), requires_grad=False).to(args.device)
            ...
            if depend_true:
                indicator = torch.tensor(np.ones(len(depend_true), dtype=np.float16),
